...
export default {
  ...
  mounted:function(){
    ...
    this.$emit('linkage',this.type,this.items[getDefaultIndex(this.items)].value)
  ...
  methods:{
    ...
    stop:function (diff) {
            ...
            // 触发选择事件
            typeof this.change === 'function' && this.change.call(_this, this.items[index], index);

            if(this.items[index].children)
              this.$emit('update:link', this.items[index].children)
           
            this.$emit('linkage',this.type,this.items[index].value)
    },
  }
}
</script>
